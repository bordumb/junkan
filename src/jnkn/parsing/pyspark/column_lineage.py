...
class ColumnLineageMapping:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "output": f"{self.output_table}.{self.output_column}" if self.output_table else self.output_column,
...
class ColumnLineageExtractor:
    ...
    def _parse_sql_with_sqlglot(self, sql: str, line_num: int) -> List[ColumnRef]:
        ...
        for select in parsed.find_all(exp.Select):
            for expr in select.expressions:
                col_refs = self._extract_columns_from_sqlglot_expr(expr, tables, line_num, ColumnContext.SELECT)
                columns.extend(col_refs)

        # Extract WHERE columns
        for where in parsed.find_all(exp.Where):
            col_refs = self._extract_columns_from_sqlglot_expr(where, tables, line_num, ColumnContext.FILTER)
            columns.extend(col_refs)
        ...
        for group in parsed.find_all(exp.Group):
            for expr in group.expressions:
                col_refs = self._extract_columns_from_sqlglot_expr(expr, tables, line_num, ColumnContext.GROUPBY)
                columns.extend(col_refs)
    ...
    def _parse_sql_with_regex(self, sql: str, line_num: int) -> List[ColumnRef]:
        ...
        # Extract SELECT columns
        select_match = re.search(r'SELECT\s+(.*?)\s+FROM', sql_normalized, re.IGNORECASE | re.DOTALL)
        if select_match:
            ...
            if select_clause.strip() != '*':
                ...
                for expr in col_exprs:
                    col_refs = self._parse_sql_column_expr(expr, tables, line_num, ColumnContext.SELECT)
                    columns.extend(col_refs)

        # Extract WHERE columns
        where_match = re.search(r'WHERE\s+(.*?)(?:GROUP|ORDER|LIMIT|$)', sql_normalized, re.IGNORECASE)
        if where_match:
            where_clause = where_match.group(1)
            col_refs = self._extract_columns_from_sql_clause(where_clause, tables, line_num, ColumnContext.FILTER)
            columns.extend(col_refs)
        ...
        for match in re.finditer(join_on_pattern, sql_normalized, re.IGNORECASE):
            ...
            col_refs = self._extract_columns_from_sql_clause(on_clause, tables, line_num, ColumnContext.JOIN)
            columns.extend(col_refs)

        # Extract GROUP BY columns
        groupby_match = re.search(r'GROUP\s+BY\s+(.*?)(?:HAVING|ORDER|LIMIT|$)', sql_normalized, re.IGNORECASE)
        if groupby_match:
            groupby_clause = groupby_match.group(1)
            col_refs = self._extract_columns_from_sql_clause(groupby_clause, tables, line_num, ColumnContext.GROUPBY)
            columns.extend(col_refs)
    ...
    def _extract_select_columns(self, source: str) -> Tuple[List[ColumnRef], List[DynamicReference]]:
    ...
    def _extract_filter_columns(self, source: str) -> Tuple[List[ColumnRef], List[DynamicReference]]:
        ...
        for match in re.finditer(filter_pattern, source, re.DOTALL):
            ...
            for var_match in re.finditer(var_pattern, args):
                ...
                if var_name not in self._variables and var_name not in ('True', 'False', 'None'):
                    dynamic.append(DynamicReference(
    ...
    def _extract_withcolumn(self, source: str) -> Tuple[List[ColumnRef], List[ColumnRef]]:
        ...
        for match in re.finditer(pattern, source, re.DOTALL):
            ...
            # Output column
            columns_written.append(ColumnRef(
                column=new_col,
    ...
    def _extract_groupby_columns(self, source: str) -> Tuple[List[ColumnRef], List[DynamicReference]]:
    ...
    def _extract_join_columns(self, source: str) -> List[ColumnRef]:
        ...
        pattern = r'\.join\s*\([^,]+,\s*(.*?)(?:,\s*["\'](?:inner|left|right|outer|cross)["\'])?\s*\)'
    ...
    def _extract_orderby_columns(self, source: str) -> Tuple[List[ColumnRef], List[DynamicReference]]:
    ...
    def _extract_col_function_refs(self, expr: str, line_num: int, context: ColumnContext) -> List[ColumnRef]:
    ...
    def _build_lineage_mappings(
        ...
        for written in columns_written:
            ...
            sources = [
                r for r in columns_read
                if r.line_number == written.line_number and
                   r.context == written.context
            ]
            ...
            if not sources:
                sources = [
                    r for r in columns_read
                    if r.context == written.context and
                       abs(r.line_number - written.line_number) <= 1
                ]

            if sources:
                mappings.append(ColumnLineageMapping(
